...
clear kk 
%channel_labels contains the labels of the channels 
...
for kk = 1:size(cc_stimsets,1)
    ...
    measured_channel_labels(cc_stimsets(kk,:)) = 0; % remove the stimulated channels
...
conn_plot = output_ER(:,:,2) < 0; % negative N1 amplitude %output_ER(:,:,1)= sample number, (:,:,2) = amplitude of the ccep

for kk = 1:size(cc_stimsets,1)
    conn_plot(cc_stimsets(kk,1:2),kk) = 0; 
end 

% % remove measured N1 in stimulated electrodes (no N1 is possible)
for kk = 1:size(cc_stimsets,1) %running through all the stimulated pairs, columns 
    for ss = 1:size(database(subj).metadata(runs).channels.name,1) %running through all the recording channels, rows  
        if cc_stimsets(kk,1) == channel_number(ss)     
...
% channel_labels:   labels of the channels, channels x 1
...
for ee = 1:size(conn_plot,1) % measured channels
   if labeled_stimsets(ss,1) ~= 0 && ~isnan(labeled_stimsets(ss,1)) % does the first stimulated electrode have a label
        % do we exclude the stimulated electrode????
...
%% Evaluate the reciprocity of connections 
